...
class Weeker:
    ...
    __day_list = ["Mon", "Thu", "Wed", "Thu", "Fri", "Sat", "Sun"]

    def __init__(self, day):

        if day not in Weeker.__day_list:
            raise WeekDayError

        self.__day_selected = day

    def __str__(self):

        return self.__day_selected

    def add_days(self, n):

        __Index_of_Day = Weeker.__day_list.index(self.__day_selected)

        __Index_of_Day += n

        while __Index_of_Day >= 7:
            __Index_of_Day -= 7

        self.__day_selected = Weeker.__day_list[__Index_of_Day]
